...
public class SecurityConfig {
    ...
    private static final String[] PUBLIC_URLS = {"/api/auth/**","/swagger-ui/**","/swagger-ui.html","/api-docs/**","/actuator/health"};
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(HttpMethod.POST,"/api/complaints/*/feedback").hasAnyRole("CUSTOMER","ADMIN").requestMatchers(HttpMethod.GET,"/api/complaints").hasAnyRole("AGENT","TEAM_LEAD","MANAGER","ADMIN").requestMatchers(HttpMethod.GET,"/api/complaints/*").hasAnyRole("CUSTOMER","AGENT","TEAM_LEAD","MANAGER","ADMIN")
    ...
    @Bean public AuthenticationProvider authenticationProvider() {
        ...
        provider.setUserDetailsService(userDetailsService); provider.setPasswordEncoder(passwordEncoder); return provider;
    }
    ...
    @Bean public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setExposedHeaders(List.of("Authorization")); config.setAllowCredentials(true); config.setMaxAge(3600L);
